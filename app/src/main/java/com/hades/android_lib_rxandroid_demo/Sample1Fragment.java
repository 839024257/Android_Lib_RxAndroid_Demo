...
//                        .subscribeOn(Schedulers.io())
    ...
    public void createDataSource4Lv(List<HashMap<String, String>> mDataSource) {
        mDataSource.add(getItemBean("Send a string", "btn0"));
        mDataSource.add(getItemBean("Send a string. Observable.just is a simplification for Observable.create.", "start"));
        mDataSource.add(getItemBean("map的功能就是在observable和subscribe之间可以对数据进行操作", "btn2"));
        mDataSource.add(getItemBean("[RxJava] Observer as receiver.", "btn3"));
        mDataSource.add(getItemBean("[RxJava] How Observable to emit the story from API server..", "btn4"));
        mDataSource.add(getItemBean("[RaJava] Schedulers决定了Observables在哪个线程发射他们的异步数据流，也决定了Observers在哪个线程消费这些数据流。", "btn5"));
        mDataSource.add(getItemBean("[RxJava] The truth of position5 : 一个操作符是被应用于一个源Observable并且作为应用的结果它将返回一个新的Observable。", "btn6"));
        mDataSource.add(getItemBean("[RxJava] map操作符允许我们把一个发射故事的Observable变成一个发射这些故事标题的Observable。", "btn7"));
        mDataSource.add(getItemBean("[RxJava] 在I/O线程发射HackerNews的故事, 但是将这些故事的标题传递给UI线程上的Observers。", "btn8"));
    }
